...
/** Used for scheduling delayed callbacks to parts. Do not use this for redstone
  * applications that require precise timing. If 2 parts are both scheduled for
  * an update on the same tick, there is no guarantee which one will update
  * first. These parts should not depend on a state of another part that may
  * have changed before/after them.
  */
object TickScheduler extends WorldExtensionInstantiator {
  ...
  private class WorldTickScheduler(world$ : World)
      extends WorldExtension(world$) {
    var schedTime = 0L
    ...
    def _scheduleTick(part: TMultiPart, time: Long, random: Boolean) {
      if (part.tile != null)
        getChunkExtension(part.tile.xCoord >> 4, part.tile.zCoord >> 4)
          .asInstanceOf[ChunkTickScheduler]
          .scheduleTick(part, time, random)
    }

    def loadRandom(part: TRandomUpdateTick) =
      scheduleTick(part, nextRandomTick, true)
    ...
    def saveDir: File = {
      ...
      ) // Calling DimensionManager.getCurrentSaveRootDirectory too early breaks game saves, we have a world reference, use it
  ...
  def createWorldExtension(world: World): WorldExtension =
    new WorldTickScheduler(world)

  private class ChunkTickScheduler(chunk$ : Chunk, world: WorldTickScheduler)
      extends ChunkExtension(chunk$, world) {
    ...
    def scheduleTick(part: TMultiPart, time: Long, random: Boolean) {
      ...
      while (it.hasNext) {
        ...
        if (e.part == part) {
          if (
            e.random && !random
          ) // only override an existing tick if we're going from random->scheduled
            {
              e.time = time
              e.random = random
            }
  ...
  def createChunkExtension(
      chunk: Chunk,
      world: WorldExtension
  ): ChunkExtension =
    new ChunkTickScheduler(chunk, world.asInstanceOf[WorldTickScheduler])

  /** Start random ticking for a part. Should be called from
    * TMultiPart.onWorldJoin
    */
  def loadRandomTick(part: TRandomUpdateTick) {
    getExtension(part.tile.getWorldObj)
      .asInstanceOf[WorldTickScheduler]
      .loadRandom(part)
  }
  ...
  def scheduleTick(part: TMultiPart, ticks: Int) {
    getExtension(part.tile.getWorldObj)
      .asInstanceOf[WorldTickScheduler]
      .scheduleTick(part, ticks, false)
  }

  /** Returns the current scheduler time. Like the world time, but unaffected by
    * the time set command and other things changing time of day. Deprecated in
    * favor of world.getTotalWorldTime
    */
  @Deprecated
  def getSchedulerTime(world: World): Long =
    getExtension(world).asInstanceOf[WorldTickScheduler].schedTime
}
