...
class TileMultipart extends TileEntity with IChunkLoadTile {
  ...
  /** Notifies all parts not in the passed collection of a change from all the
  ...
  /** Helper function for calling a second level notify on a side (eg indirect
    * power from a lever)
    */
  ...
  /** Returns true if opart can be replaced with npart (note opart and npart may
    * be the exact same object)
    *
    * This function should be used for testing if a part can change it's shape
    * (eg. rotation, expansion, cable connection) For example, to test whether a
    * cable part can connect to it's neighbor:
    *   1. Set the cable part's bounding boxes as if the connection is
    *      established 2. Call canReplacePart(part, part) 3. If canReplacePart
    *      succeeds, perform connection, else, revert bounding box
    */
  ...
  def occlusionTest(parts: Seq[TMultiPart], npart: TMultiPart): Boolean = {
    return parts.forall(part =>
      part.occlusionTest(npart) && npart.occlusionTest(part)
    )
  }

  /** Get the write stream for updates to part
    */
  def getWriteStream(part: TMultiPart): MCDataOutput =
    getWriteStream.writeByte(partList.indexOf(part))
  ...
  private[multipart] def addPart_do(part: TMultiPart) {
    assert(
      partList.size < 250,
      "Tried to add more than 250 parts to the one tile. You're doing it wrong"
    )
  ...
  /** Bind this part to an internal cache. Provided for trait overrides, do not
    * call externally.
    */
  def bindPart(part: TMultiPart) {}

  /** Called when a part is added (placement) Provided for trait overrides, do
    * not call externally.
    */
  def partAdded(part: TMultiPart) {}

  /** Removes part from this tile. Note that due to the operation sync, the part
    * may not be removed until the call stack has been passed to all other parts
    * in the space.
    */
  ...
  /** Remove this part from internal cache. Provided for trait overrides, do not
    * call externally.
    */
  ...
  /** Perform a raytrace returning all intersecting parts sorted nearest to
    * farthest
    */
  ...
  def collisionRayTrace(start: Vec3, end: Vec3): ExtendedMOP =
    rayTraceAll(start, end).headOption.getOrElse(null)

  /** Drop and remove part at index (internal mining callback)
    */
  def harvestPart(index: Int, hit: ExtendedMOP, player: EntityPlayer) =
...
object TileMultipart {
  ...
  /** Gets a multipar ttile instance at pos, converting if necessary.
    */
  def getOrConvertTile(world: World, pos: BlockCoord) =
    getOrConvertTile2(world, pos)._1

  /** Gets a multipart tile instance at pos, converting if necessary. Note
    * converted tiles are merely a structure formality, they do not actually
    * exist in world until they are required to by the addition of another
    * multipart to their space.
    * @return
    *   (The tile or null if there was none, true if the tile is a result of a
    *   conversion)
    */
  def getOrConvertTile2(
      world: World,
      pos: BlockCoord
  ): (TileMultipart, Boolean) = {
    val t = world.getTileEntity(pos.x, pos.y, pos.z)
    if (t.isInstanceOf[TileMultipart])
      return (t.asInstanceOf[TileMultipart], false)

    val p = MultiPartRegistry.convertBlock(
      world,
      pos,
    ...
    if (p != null) {
      val t =
        MultipartGenerator.generateCompositeTile(null, Seq(p), world.isRemote)
      t.xCoord = pos.x
      t.yCoord = pos.y
      t.zCoord = pos.z
      t.setWorldObj(world)
  ...
  /** Gets the multipart tile instance at pos, or null if it doesn't exist or is
    * not a multipart tile
    */
  ...
  def checkNoEntityCollision(world: World, pos: BlockCoord, part: TMultiPart) =
    part.getCollisionBoxes.forall(b =>
      world.checkNoEntityCollision(b.toAABB.offset(pos.x, pos.y, pos.z))
    )

  /** Returns whether part can be added to the space at pos. Will do conversions
    * as necessary. This function is the recommended way to add parts to the
    * world.
    */
  ...
  def replaceable(world: World, pos: BlockCoord): Boolean = {
    ...
    return block.isAir(world, pos.x, pos.y, pos.z) || block.isReplaceable(
      world,
      pos.x,
      pos.y,
      pos.z
